...
for ((octet=1;octet<=255;octet++)); do
    skip_address=$(egrep -L "\b((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|$)){3}${octet}\b" ./*.out)
...
# ls -1 *.out | xargs -I {} egrep -H "\b((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|$)){3}23\b" {} | sort -u -t":" -k1,1|sed 's/\.out//;s/:/ /'
